"""
Real-Time Candlestick Simulation App
...
Features:
- Growing candlesticks with 1-second resolution
- Configurable candle colors
- Multiple timeframes (1s, 1m, 5m)
- Technical indicators (VWAP, 9 EMA, 20 EMA)
- Volume with average overlay
- Bid/Ask/Last/Spread price panel
- Playback controls with jump functionality
- Session-aware display (Pre-market, RTH, After-hours)
...
def main():
    st.set_page_config(
        page_title="NQ Futures Real-Time Simulation",
        page_icon="📊",
        layout="wide",
    ...
    # Title
    st.title("📊 NQ Futures Real-Time Candlestick Simulation")
        ...
        if st.session_state.current_timestamp:
            ...
            st.markdown(
                f"""<div style='display: flex; align-items: center; gap: 10px;'>
                    <span class='session-badge' style='background-color: {session_info["color"]};'>
                        {session_info['label']}
                    </span>
    ...
    with col2:
        if tick:
            st.metric("Last", f"${tick.get('price', 0):.2f}")
    ...
    with control_cols[6]:
        if st.button("⏪ Jump Back", use_container_width=True):
            if st.session_state.current_timestamp:
                ...
                if jump == 'session':
                    # Jump to start of current session
                    session = get_current_session(st.session_state.current_timestamp)
                    ...
                    # Jump to start of day (pre-market)
                    st.session_state.current_timestamp = datetime.combine(
                        st.session_state.current_timestamp.date(),
                        time(4, 0)
                    )
    ...
    with control_cols[7]:
        if st.button("⏩ Jump Forward", use_container_width=True):
            if st.session_state.current_timestamp:
                jump = next((opt[1] for opt in JUMP_OPTIONS if opt[0] == jump_amount), timedelta(minutes=1))
                if jump == 'session':
                    # Jump to end of current session / start of next
                    session = get_current_session(st.session_state.current_timestamp)
                    if session in SESSIONS:
                        st.session_state.current_timestamp = datetime.combine(
                            st.session_state.current_timestamp.date(),
    ...
    if st.session_state.is_playing:
        # Calculate refresh interval based on playback speed
        # At 1x speed, refresh every 1 second
        # At 100x speed, refresh every 10ms (100 updates per second would be too fast, cap at ~10 fps)
        refresh_interval = max(100, int(1000 / st.session_state.playback_speed))
        st_autorefresh(interval=refresh_interval, limit=None, key="auto_refresh")
        
        # Advance timestamp
        if st.session_state.current_timestamp and st.session_state.data_loader:
            # Move forward by playback_speed seconds
    ...
    # Load current tick and update candles
    if st.session_state.current_timestamp and st.session_state.data_loader:
    ...
    if not st.session_state.candles.empty:
        ...
        if chart_option:
            st_echarts(chart_option, height="600px")
    else:
