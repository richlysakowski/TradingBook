@echo off
REM ============================================================================
REM TradingBook - Build Portable Version Script (Optimized)
REM ============================================================================
REM Builds the portable Windows executable with smart caching.
REM
REM Usage:
REM   build_portable.bat           Smart build (uses cache, ~3-4 min)
REM   build_portable.bat --force   Clean rebuild (no cache, ~8-12 min)
REM
REM Output: portable_version_kit\TradingBook X.X.X.exe
REM ============================================================================
...
echo.
echo ============================================================================
echo  TradingBook - Portable Version Builder
echo ============================================================================
if !FORCE_BUILD!==1 (
...
REM ============================================================================
REM Step 2: Prepare output folder (~1 second)
REM ============================================================================
echo [Step 2/5] Preparing output folder...

if not exist "portable_version_kit" (
    mkdir portable_version_kit
    echo   [CREATED] portable_version_kit folder
) else (
    echo   [OK] portable_version_kit folder exists
)
echo.

REM ============================================================================
REM Step 3: Install dependencies (SKIP if node_modules exists)
REM ============================================================================
...
REM ============================================================================
REM Step 4: Build React bundle (SKIP if build/ exists and not --force)
REM ============================================================================
echo [Step 4/5] React production bundle...
...
REM ============================================================================
REM Step 5: Build portable executable (~3-5 minutes)
REM ============================================================================
echo [Step 5/5] Building portable executable...
echo   [INFO] This takes 3-5 minutes...
echo   [INFO] electron-builder is packaging the application...
echo.
...
if !ERRORLEVEL! neq 0 (
    ...
    echo   3. Try again with: build_portable.bat --force
    exit /b 1
)

echo   [OK] Portable executable built
echo.

REM ============================================================================
REM Copy to portable_version_kit
REM ============================================================================
echo [Finalizing] Copying to portable_version_kit...

set FOUND_EXE=0
for %%f in (dist\TradingBook*.exe) do (
    echo %%~nxf | findstr /i "Setup" >nul
    if !ERRORLEVEL! neq 0 (
        copy "%%f" "portable_version_kit\" >nul 2>&1
        echo   [OK] Copied %%~nxf
        set FOUND_EXE=1
    )
)
...
if !FOUND_EXE!==0 (
    echo   [WARNING] No portable .exe found in dist folder
)
...
echo  Output:   portable_version_kit\
echo.
echo  Files:
dir /b portable_version_kit\*.exe 2>nul
echo.
echo  Next: Test the .exe, then distribute with QuickStart guide
echo  Data location: %%APPDATA%%\tradingbook\
echo ============================================================================
echo.

REM Open the output folder
start portable_version_kit
